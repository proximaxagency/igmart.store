...
export default function AdminSettingsPage() {
  ...
  const runSeed = async (fn: () => Promise<unknown>, label: string) => {
    ...
    try {
      ...
      setSeedStatus(`✅ ${label}: ${JSON.stringify(result)}`);
    } catch (e: unknown) {
      setSeedStatus(`❌ Error: ${e instanceof Error ? e.message : String(e)}`);
    } finally {
      setSeeding(false);
    }
  };
        ...
        <div className="p-6 space-y-4">
          ...
          {seedStatus && (
            <div className={`text-xs font-mono px-4 py-3 rounded-xl border ${
              seedStatus.startsWith("✅") ? "bg-green-500/10 border-green-500/30 text-green-400" : "bg-red-500/10 border-red-500/30 text-red-400"
            }`}>
              {seedStatus}
            </div>
          )}
