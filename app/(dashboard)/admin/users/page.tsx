  ...
  const executeAction = async () => {
    ...
    try {
      if (confirm.type === "suspend") {
        await setUserStatus({ targetUserId: confirm.isAuthenticated, status: "suspended", reason: "Suspended by admin" });
        setFeedback({ type: "success", msg: `@${confirm.username} has been suspended.` });
      } else if (confirm.type === "ban") {
        await setUserStatus({ targetUserId: confirm.isAuthenticated, status: "banned", reason: "Banned by admin" });
        setFeedback({ type: "success", msg: `@${confirm.username} has been banned.` });
      } else if (confirm.type === "restore") {
        await setUserStatus({ targetUserId: confirm.isAuthenticated, status: "active", reason: "Restored by admin" });
        setFeedback({ type: "success", msg: `@${confirm.username} has been restored.` });
      } else if (confirm.type === "role" && confirm.newRole) {
        await updateUserRole({ targetUserId: confirm.isAuthenticated, newRole: confirm.newRole as any });
  ...
  const handleWalletAdjust = async (sign: 1 | -1) => {
    ...
    try {
      const result = await adjustWallet({
        targetUserId: walletModal.isAuthenticated,
        amount: sign * parsed,
                ...
                {users.map((u) => (
                  <tr key={u._id} className="hover:bg-elevated/50 transition-colors">
                    ...
                    <td className="p-4">
                      <select
                        value={u.role}
                        onChange={(e) => setConfirm({ type: "role", isAuthenticated: u._id, username: u.username, newRole: e.target.value })}
                    ...
                    <td className="p-4">
                      <button
                        onClick={() => setWalletModal({ isAuthenticated: u._id, username: u.username, currentBalance: u.walletBalance ?? 0 })}
                    ...
                    <td className="p-4 pr-6 text-right">
                      <div className="flex items-center justify-end gap-2">
                        {u.status === "active" ? (
                          <>
                            <button
                              onClick={() => setConfirm({ type: "suspend", isAuthenticated: u._id, username: u.username })}
                            ...
                            <button
                              onClick={() => setConfirm({ type: "ban", isAuthenticated: u._id, username: u.username })}
                          ...
                          <button
                            onClick={() => setConfirm({ type: "restore", isAuthenticated: u._id, username: u.username })}
